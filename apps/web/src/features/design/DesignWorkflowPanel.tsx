...
export function useDesignWorkflows() {
  return useQuery({
    queryKey: ["design-workflows"],
    queryFn: () => api.designWorkflows(),
    staleTime: Infinity,
  });
}
...
export function DesignWorkflowPanel({ kind, defaultOpen = false }: { kind: DesignKind; defaultOpen?: boolean }) {
  const { data: workflows = [] } = useDesignWorkflows();
  ...
  return (
    <div className="rounded-lg border border-line bg-surface">
      ...
      >
        ...
        <DownOutlined
          className={`ml-auto text-[10px] text-faint transition-transform ${open ? "" : "-rotate-90"}`}
        />
      </button>
