...
export function BoardPage() {
  ...
  const createSprint = async () => {
    ...
    const s = await api.createBoardSprint(currentId, newSprintName.trim());
    setNewSprintName("");
  ...
  const toggleAuto = async (id: string, enabled: boolean) => {
    ...
    await qc.invalidateQueries({ queryKey: ["board-autos", currentId] });
  };
  ...
  const addAuto = async () => {
    if (!currentId || !autoTrigger.trim()) {
      return;
    }
    await api.createBoardAutomation(currentId, autoTrigger.trim(), "run");
    setAutoTrigger("");
    await qc.invalidateQueries({ queryKey: ["board-autos", currentId] });
  };
  ...
  const columnCards = (status: BoardStatus) =>
    cards
      .filter((c) => c.status === status && !isStageSub(c) && inScope(c))
          ...
          <div className="flex gap-1 text-sm">
            {(["sprint", "backlog", "automations"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`border-b-2 px-2 py-1 capitalize ${tab === t ? "border-accent font-medium text-fg" : "border-transparent text-muted"}`}
              >
                {t}
              </button>
            ))}
          ...
          {tab === "sprint" && (
            <div className="ml-auto flex items-center gap-2">
              <Select
                size="small"
                value={sprintFilter}
                onChange={(v) => setSprintFilter(v as string)}
                style={{ width: 180 }}
              ...
              {newSprintOpen ? (
                <span className="flex items-center gap-1">
                  <Input
                    ...
                    onPressEnter={createSprint}
                    placeholder="Sprint name"
                    style={{ width: 150 }}
                  />
                  <Button size="small" type="primary" onClick={createSprint}>
                    Add
                  </Button>
                  <Button
                    size="small"
                ...
                <Button size="small" onClick={() => setNewSprintOpen(true)}>
                  + New sprint
                </Button>
              )}
            </div>
      ...
      {!currentId ? (
        ...
        <div className="max-w-2xl">
          <p className="mb-3 text-sm text-muted">
            When a card gets a matching label, the board hands it to an agent automatically. Trigger format{" "}
            <code>label:name</code>.
          </p>
          <div className="flex flex-col gap-2">
            {automations.length === 0 && <Empty description="No automations yet." />}
            {automations.map((a) => (
              <div key={a.id} className="flex items-center gap-3 rounded-lg border border-line bg-surface p-3">
                <Switch checked={a.enabled} onChange={(v) => toggleAuto(a.id, v)} />
                <span className="text-sm font-medium">Labeled {a.trigger.replace("label:", "")} → run the task</span>
                <span className="ml-auto font-mono text-xs text-faint">
                  {a.trigger} → {a.action}
                </span>
                <Button size="small" danger onClick={() => deleteAuto(a.id)}>
                  Delete
                </Button>
              </div>
            ))}
          ...
          <div className="mt-4 flex items-center gap-2 border-t border-line pt-4">
            <Input
              ...
              onPressEnter={addAuto}
              placeholder="label:design"
              style={{ width: 160 }}
            />
            <span className="text-faint">→ run</span>
            <Button size="small" type="primary" onClick={addAuto}>
              Add automation
            </Button>
          </div>
        </div>
